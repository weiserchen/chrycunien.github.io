...
### Adaptor
In the example, we are given a `VectorImage` and we want to adapt to a `RasterImage` in order to output to console.
...
```
- This is the desired interfaces
```go
type Point struct {
  X, Y int
}
...
### Caching
We use a cache to store the results.
